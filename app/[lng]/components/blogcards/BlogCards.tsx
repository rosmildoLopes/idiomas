...
const BlogCards = async ({ lng }) => {
  const { t } = await useTranslation(lng, 'blogs', { returnObjects: true });
  const blogData = t('blogs', { returnObjects: true });

  // Verificar si lee blogData como un array antes de mapearlo
  if (!Array.isArray(blogData)) {
    
    return <div>Error: Los datos del blog no están disponibles.</div>;
  }
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-y-14 gap-x-12">
        {blogData.map((blog) => (
          ...
          >
            ...
            <Image src={blog.image} alt={blog.title} className="mb-2 p-2" />
            <h2
